...
public class dibuixConstantMult extends JFrame implements Comunicar{
    ...
    public dibuixConstantMult(Main main) {
        ...
        for (int i = 0; i < taules.length; i++) {
            ...
            models[i] = new DefaultTableModel(new Object[]{"n", "Temps Real (ms)", "Temps Esperat (ms)", "Constant Multiplicativa"}, 0) {
                @Override
    ...
    public synchronized void afegirFilaSuma(int n, String real, String esperat, String cm){
        models[INDEX_SUMA].addRow(new Object[]{n, real, esperat, cm});
        taules[INDEX_SUMA].setVisible(true);
        revalidate();
        repaint();
    }

    public synchronized void afegirFilaMutl(int n, String real, String esperat, String cm){
        models[INDEX_MULT].addRow(new Object[]{n, real, esperat, cm});
